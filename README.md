...
```bash
npm install @ohcnetwork/care-filly-character three @react-three/fiber
```
...
(From a sibling checkout: `npm install ../care_filly_character` — run `npm run build` here first.)
...
<FillyMascot status={filly.status} speaking={isAssistantSpeaking} audioLevel={level} size={96} />
```
...
const level = barHeights.reduce((a, b) => a + b, 0) / Math.max(1, barHeights.length);

<React.Suspense fallback={null}>
  <FillyMascot status={filly.status} audioLevel={level} size={96} onClick={handleStart} />
</React.Suspense>
```
...
a 26° camera at `(0, 0.22, 6.9)` looking at `(0, -0.02, 0)`. Set `scene.environment` to a
...
- **Rendering:** transparent canvas, `dpr ≤ 2`, hemisphere + key/fill/rim lights, procedural
  `RoomEnvironment`, `NeutralToneMapping`. No shadow maps — the contact shadow is a gradient plane.
- **Cost:** one-time CSG build ≈ 60–80 ms (cached module-wide); ~15 k triangles per character;
